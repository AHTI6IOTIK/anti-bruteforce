...
func ParseConfig(v *viper.Viper) (*Config, error) {
	...
	c.Redis.Host = v.GetString(envNameConfigRedisHost)
	if c.Redis.Host == "" {
		v.SetDefault(envNameConfigRedisHost, "0.0.0.0")
		c.Redis.Host = v.GetString(envNameConfigRedisHost)
	}
	c.Redis.Port = v.GetString(envNameConfigRedisPort)
	if c.Redis.Port == "" {
		c.Redis.Port = "6379"
	}
	c.Redis.Password = v.GetString(envNameConfigRedisCrdntls)
